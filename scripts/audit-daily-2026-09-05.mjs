...
const pages=[
 {path:"guides/rosamond-gifford-zoo-with-kids.html",title:"Rosamond Gifford Zoo With Kids: The Two-Loop Day That Leaves Before the Crash",canonical:"https://mradventuredad.com/guides/rosamond-gifford-zoo-with-kids.html",photo:"rosamond-gifford-zoo.webp",paid:0,sources:["rosamondgiffordzoo.org/visit/plan-your-visit/hours-and-admission","rosamondgiffordzoo.org/visit/plan-your-visit/parking-and-accessibility","rosamondgiffordzoo.org/visit/food-and-shopping"]},
 {path:"guides/family-hearing-protection-earmuffs-earplugs.html",title:"Family Hearing Protection: Earmuffs vs Earplugs—and Why NRR Is Not a Fit Test",canonical:"https://mradventuredad.com/guides/family-hearing-protection-earmuffs-earplugs.html",photo:"family-hearing-protection.webp",paid:3,sources:["nidcd.nih.gov/health/noise-induced-hearing-loss","cdc.gov/niosh/noise/prevent/ppe.html","epa.gov/clean-air-act-overview/clean-air-act-title-iv-noise-pollution"]},
 {path:"guides/family-outdoor-weather-cutoff-plan.html",title:"The Family Outdoor Weather Cutoff Plan: Go, Shorten, or Leave",canonical:"https://mradventuredad.com/guides/family-outdoor-weather-cutoff-plan.html",photo:"family-weather-decision.webp",paid:0,sources:["weather.gov/safety/lightning","weather.gov/safety/heat-ww","airnow.gov/education/students/aqi-for-ozone"]}
];
...
for(const p of pages){
 ...
 if(!h.includes('datePublished":"2026-09-05"')||!h.includes('dateModified":"2026-09-05"'))fail(`${p.path}: structured dates missing`);
...
for(const [hub,targets] of Object.entries({"index.html":pages.map(p=>p.path),"adventures.html":pages.map(p=>p.path),"destinations.html":[pages[0].path],"outdoors.html":[pages[2].path],"gear.html":[pages[1].path,pages[2].path]})){const h=read(hub);for(const t of targets)if(!h.includes(`href="${t}"`))fail(`${hub}: missing ${t}`);}
for(const [path,target] of Object.entries({"guides/syracuse-family-day-trip-guide.html":"rosamond-gifford-zoo-with-kids.html","guides/amusement-park-survival-plan.html":"family-hearing-protection-earmuffs-earplugs.html","guides/finger-lakes-rainy-day-plan.html":"family-outdoor-weather-cutoff-plan.html"}))if(!read(path).includes(`href="${target}"`))fail(`${path}: missing related ${target}`);
if(failures.length){console.error(failures.map(x=>`FAIL ${x}`).join("\n"));process.exit(1);} console.log("PASS daily 2026-09-05: 3 substantial pages, 3 photographic heroes, 0 charts, 3 disclosed Amazon links, authoritative sources, discovery and related links verified.");
